...
def offices():
    ...
    try:
        ...
        update_sql = f"""
            ...
            WHERE s.officeCode = d.officeCode
              AND s.update_timestamp >= '{ETL_BATCH_DATE}';
        """
        cur.execute(update_sql)
        print(f"Updated offices: {cur.rowcount}")
        ...
        insert_sql = f"""
            ...
            SELECT 
                s.officeCode, s.city, s.phone, s.addressLine1, s.addressLine2,
                s.state, s.country, s.postalCode, s.territory,
            ...
            WHERE s.create_timestamp >= '{ETL_BATCH_DATE}'
              AND d.officeCode IS NULL;
        """
        cur.execute(insert_sql)
        print(f"Inserted offices: {cur.rowcount}")

        conn.commit()
        print("Offices ETL completed successfully.")

    except Exception as e:
        conn.rollback()
        print("Error during Offices ETL:", str(e))
